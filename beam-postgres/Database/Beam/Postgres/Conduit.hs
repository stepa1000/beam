...
import Database.Beam hiding (runDelete, runInsert, runUpdate)
...
streamingRunSelect ::
  (C.MonadResource m, Fail.MonadFail m, FromBackendRow Postgres a) =>
  Pg.Connection ->
...
runSelect ::
  (MonadIO m, Fail.MonadFail m, MonadBaseControl IO m, FromBackendRow Postgres a) =>
  Pg.Connection ->
  SqlSelect Postgres a ->
...
streamingRunQueryError ::
  (C.MonadResource m, Fail.MonadFail m) =>
  Pg.Connection ->
  PgSyntax ->
  CONDUIT_TRANSFORMER () r m String
streamingRunQueryError (conn@Pg.Connection {connectionHandle}) x = do
...
runInsertWithError ::
  (C.MonadResource m, Fail.MonadFail m, C.MonadUnliftIO m) =>
  Pg.Connection ->
  SqlInsert Postgres tbl ->
  m String
runInsertWithError conn (SqlInsert _ (PgInsertSyntax i)) = C.runConduitRes $ streamingRunQueryError conn i
    ...
    Just row ->
      liftIO (Pg.resultStatus row)
        >>= \case
          ...
          _ -> do
            errMsg <- liftIO (Pg.resultErrorMessage row)
